    ...
    <div className="relative flex items-center gap-3 md:gap-4">
        ...
        {items.slice(0, 7).map((item) => {
          ...
          return (
            <li key={item.href}>
              <Link
                href={item.href}
                className="nav-link font-medium cursor-pointer w-fit overflow-hidden whitespace-nowrap text-base text-black"
                style={{
      ...
      {moreItems.length > 1 && (
        <div className="z-100 flex ">
          <Menu>
            <MenuButton
              className={`nav-link group outline-none cursor-pointer flex items-center gap-1 xl:gap-1.5 font-medium text-base rtl:leading-10 whitespace-nowrap lg:p-2 lg:pe-5 duration-300 rounded-full text-white
